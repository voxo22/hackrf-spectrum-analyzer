...
public class MVCController {
    ...
    public <T> MVCController(JTextField textField, ModelValue<T> model,
                             Function<String, T> conversionViewToModel,
                             Function<T, String> conversionModelToView) {

        if (textField == null || model == null || conversionViewToModel == null || conversionModelToView == null) {
            throw new IllegalArgumentException("MVCController: One or more arguments are null");
        }

        textField.addActionListener(e -> updateModelValue(() -> {
            try {
                model.setValue(conversionViewToModel.apply(textField.getText()));
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }));
        ...
        model.addListener(() -> updateView(() -> {
            try {
                textField.setText(conversionModelToView.apply(model.getValue()));
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }));
	...
	public MVCController(JTextField textField, ModelValue<String> model) {
	    this(textField, model, s -> s, s -> s);
	}
