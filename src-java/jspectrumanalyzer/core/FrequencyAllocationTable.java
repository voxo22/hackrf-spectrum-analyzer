...
//----
		private double mapToCompressedAxis(double originalFreq, int[] ranges) {
		    double currentStart = ranges[0];       // začiatok pôvodného rozsahu
		    double compressedPos = 0;             // pozícia na komprimovanej osi
		
		    for (int i = 0; i < ranges.length; i += 2) {
		        double blockStart = ranges[i];
		        double blockEnd = ranges[i + 1];
		        double blockLength = blockEnd - blockStart;
		
		        if (originalFreq >= blockStart && originalFreq <= blockEnd) {
		            // Hodnota spadá do tohto bloku
		            return compressedPos + (originalFreq - blockStart);
		        }
		
		        // Ak nie, posuň komprimovanú pozíciu o dĺžku bloku
		        compressedPos += blockLength;
		    }
		
		    return originalFreq; // fallback, ak mimo rozsah
		}
//----
	...
	public BufferedImage drawAllocationTable(int width, int height, float alpha, long freqStartHz, long freqEndHz, Color textColor, Color bgColor) {
		...
		Font font = new Font(Font.DIALOG, Font.PLAIN, 12*scale);
		...
		final float textOffset	= fontHeight/2;
		...
		class BandRectangle{
			...
			public String color;
			public BandRectangle(FrequencyBand band, ArrayList<String> lines, float x, float y, float w, String longestString, String color) {
				this.band	= band;
				this.lines = lines;
				this.x = x;
				this.y = y;
...
//----		
		int[] rozsah = {920, 925, 940, 945, 955, 960};
		double compressedLength = 0;
		for (int l = 0; l < rozsah.length; l += 2) {
		    compressedLength += (rozsah[l + 1] - rozsah[l]);
		}
//----
...
/*
			float xStart = (float) (mapToCompressedAxis(band.getMHzStartIncl(), rozsah) * drawWidth / compressedLength);
			float xEnd   = (float) (mapToCompressedAxis(band.getMHzEndExcl(), rozsah) * drawWidth / compressedLength);
			shapes.sort(Comparator.comparingDouble(shape -> shape.x));
*/
			String color = band.getColor();
			if (xStart < 0)
			...
			shapes.add(new BandRectangle(band, listLines, x, y, w, lines[0], color));
		}
		...
		for (int j = 0; j < shapes.size(); j++) {
			...
			if (shape.w > bounds.getWidth()) {
				...
				shape.textVisible	= true;
			}
		}
		//System.out.println("visible: " + maxVisibleLines);
		float rectHeight	= (maxVisibleLines) * fontHeight + textOffset;
...
//				g.setColor(Color.white);
				...
				for (int line = 0; line < shape.lines.size(); line++) {
					...
					gg.drawString(linestr, textX, /*textOffset +*/ (line+1)*fontHeight);
				}
				gg.dispose();
			}
			/*
		    ...
		    */
	        ...
	        g.drawLine((int)shape.x, 0, (int)shape.x, 450);
	        g.drawLine((int)shape.x + (int)shape.w, 0, (int)shape.x + (int)shape.w, 450);
		}
